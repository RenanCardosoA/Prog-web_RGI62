...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(empty($nome)||empty($email)){ $errorMessage = "Nome e E-mail são obrigatórios."; }
    else{
        if(!empty($senha)){
            $senha_hash = password_hash($senha,PASSWORD_DEFAULT);
            $stmt = $conn->prepare("UPDATE usuario SET nome=:nome, email=:email, tipo=:tipo, senha_hash=:senha WHERE id_usuario=:id");
            $stmt->execute([':nome'=>$nome, ':email'=>$email, ':tipo'=>$tipo, ':senha'=>$senha_hash, ':id'=>$id_usuario]);
        } else{
            $stmt = $conn->prepare("UPDATE usuario SET nome=:nome, email=:email, tipo=:tipo WHERE id_usuario=:id");
            $stmt->execute([':nome'=>$nome, ':email'=>$email, ':tipo'=>$tipo, ':id'=>$id_usuario]);
        }
